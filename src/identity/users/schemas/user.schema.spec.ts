...
describe('UserSchema', () => {
  it('should generate a default uid with usr_ prefix using nanoid', () => {
    const uidPath = UserSchema.path('uid') as any;
    expect(uidPath).toBeDefined();

    const generatedUid = typeof uidPath.defaultValue === 'function' ? uidPath.defaultValue() : '';
  ...
  it('should have default roles as [UserRole.USER]', () => {
    ...
    const defaultRoles = typeof rolesPath.defaultValue === 'function' ? rolesPath.defaultValue() : rolesPath.defaultValue;
    expect(defaultRoles).toEqual([UserRole.USER]);
  });

  it('should have default status as PENDING_VERIFICATION', () => {
    const statusPath = UserSchema.path('status') as any;
    expect(statusPath).toBeDefined();

    const defaultStatus = typeof statusPath.defaultValue === 'function' ? statusPath.defaultValue() : statusPath.defaultValue;
